...
QtStateMachinePrivate::QtStateMachinePrivate()
{
    ...
    globalRestorePolicy = QtAbstractState::DoNotRestoreProperties;
    rootState = 0;
    ...
    for (it = configuration.constBegin(); it != configuration.constEnd(); ++it) {
        ...
        for (int j = 0; (j < lst.size()) && !found; ++j) {
            ...
            for (int k = 0; k < transitions.size(); ++k) {
                ...
                if (QtAbstractTransitionPrivate::get(t)->callEventTest(event)) {
#ifdef QSTATEMACHINE_DEBUG
...
void QtStateMachinePrivate::microstep(const QList<QtAbstractTransition*> &enabledTransitions)
{
    ...
    QList<QtAbstractState*> enteredStates = enterStates(enabledTransitions);
...
QList<QtAbstractState*> QtStateMachinePrivate::exitStates(const QList<QtAbstractTransition*> &enabledTransitions)
{
...
QList<QtAbstractState*> QtStateMachinePrivate::enterStates(const QList<QtAbstractTransition*> &enabledTransitions)
{
#ifdef QSTATEMACHINE_DEBUG
    Q_Q(QtStateMachine);
#endif
...
void QtStateMachinePrivate::applyProperties(const QList<QtAbstractTransition*> &transitionList,
                                           const QList<QtAbstractState*> &enteredStates)
{
#ifndef QT_NO_ANIMATION
    Q_Q(QtStateMachine);
    ...
    for (int i = 0; i < enteredStates.size(); ++i) {
        QtAbstractState *s = enteredStates.at(i);

        QtAbstractState::RestorePolicy restorePolicy = s->restorePolicy();
        if (restorePolicy == QtAbstractState::GlobalRestorePolicy)
            restorePolicy = globalRestorePolicy;

        QList<QPropertyAssignment> assignments = QtAbstractStatePrivate::get(s)->propertyAssignments;
        for (int j = 0; j < assignments.size(); ++j) {
            const QPropertyAssignment &assn = assignments.at(j);
            if (restorePolicy == QtAbstractState::RestoreProperties) {
...
QList<QPropertyAssignment> QtStateMachinePrivate::restorablesToPropertyList(const QHash<RestorableId, QVariant> &restorables) const
{
...
QtAbstractState *QtStateMachinePrivate::findErrorState(QtAbstractState *context)
{
    ...
    QtAbstractState *errorState = 0;
    QtState *s = qobject_cast<QtState*>(context);
    if (s) {
...
QtStateMachinePrivate::initializeAnimation(QAbstractAnimation *abstractAnimation, 
...
void QtStateMachinePrivate::_q_animationFinished()
{
    Q_Q(QtStateMachine);
...
{
public:
    InitialTransition(QtAbstractState *target)
...
void QtStateMachinePrivate::_q_start()
{
    ...
    QtAbstractTransition *initialTransition = new InitialTransition(initial);
    ...
    applyProperties(transitions, QList<QtAbstractState*>() << initial);
    delete start;
...
void QtStateMachinePrivate::unregisterTransition(QtAbstractTransition *transition)
{
    if (QtSignalTransition *st = qobject_cast<QtSignalTransition*>(transition)) {
        unregisterSignalTransition(st);
    }
#ifndef QT_NO_STATEMACHINE_EVENTFILTER
    else if (QtEventTransition *oet = qobject_cast<QtEventTransition*>(transition)) {
...
void QtStateMachinePrivate::registerSignalTransition(QtSignalTransition *transition)
{
    ...
    QObject *sender = QtSignalTransitionPrivate::get(transition)->sender;
    if (!sender)
        return;
    ...
    if (signalIndex == -1) {
        qWarning("QtSignalTransition: no such signal: %s::%s",
...
void QtStateMachinePrivate::unregisterSignalTransition(QtSignalTransition *transition)
{
    int signalIndex = QtSignalTransitionPrivate::get(transition)->signalIndex;
    if (signalIndex == -1)
    ...
    if (ok) {
        ...
        QtSignalTransitionPrivate::get(transition)->signalIndex = -1;
    }
#endif
}
...
void QtStateMachinePrivate::unregisterEventTransition(QtEventTransition *transition)
{
    Q_Q(QtStateMachine);
    if (!QtEventTransitionPrivate::get(transition)->registered)
        return;
    ...
    QtEventTransitionPrivate::get(transition)->registered = false;
}
#endif
...
/*!
  \internal
*/
QtStateMachine::QtStateMachine(QtStateMachinePrivate &dd, QObject *parent)
    : QObject(
#ifndef QT_STATEMACHINE_SOLUTION
        dd,
#endif
...
/*!
   ...
   restore policy is QtAbstractState::DoNotRestoreProperties.
   
   The global restore policy cannot be set to QtAbstractState::GlobalRestorePolicy.

   \sa QtAbstractState::setRestorePolicy()
*/
...
/*!
  \fn QtStateMachine::animationsFinished()
